...
use kestrel_loom::rendering::Renderer;
...
struct CompareSymbol {
    symbol: String,
    candles: Vec<Candle>,
    color: kestrel_loom::primitives::Color,
}
...
impl WasmChart {
    ...
    fn render_compare_symbols(
        ...
        for i in 0..=4 {
            ...
            renderer.draw_text(
                ...
                axis_color,
                10.0,
                kestrel_loom::rendering::TextAlign::Right,
                kestrel_loom::rendering::TextBaseline::Middle,
            );
        }
        ...
        for (idx, (symbol, color, values)) in series.iter().enumerate() {
            ...
            if let Some((_, latest_percent)) = values.last() {
                ...
                renderer.draw_text(
                    ...
                    *color,
                    11.0,
                    kestrel_loom::rendering::TextAlign::Left,
                    kestrel_loom::rendering::TextBaseline::Top,
                );
            }
        }
    }
    ...
    fn render_footprint_candles(
        ...
        for candle in visible {
            ...
            for level in &candle.levels {
                ...
                if show_delta_text {
                    let delta = kestrel_loom::core::FootprintCandle::level_delta(level);
                    renderer.draw_text(
                        &format!("{:+.0}", delta),
                        cx,
                        y,
                        text_color,
                        8.0,
                        kestrel_loom::rendering::TextAlign::Center,
                        kestrel_loom::rendering::TextBaseline::Middle,
                    );
                }
            }
    ...
    fn render_selected_drawing_highlights(state: &ChartState, renderer: &mut Canvas2DRenderer) {
        ...
        let highlight = kestrel_loom::primitives::Color::rgba(88, 166, 255, 0.95);
    ...
    fn render_axes(
        ...
        for i in 0..=time_lines {
            ...
            renderer.draw_text(
                ...
                10.0,
                kestrel_loom::rendering::TextAlign::Center,
                kestrel_loom::rendering::TextBaseline::Bottom,
            );
        }
    }
    ...
    pub fn render(&mut self) -> Result<(), JsValue> {
        ...
        {
            use kestrel_loom::utils::bar_width::{optimal_candlestick_width, symmetric_bar_width};
            ...
            match candle_style {
                ...
                | kestrel_loom::primitives::CandleStyle::Area => {
                    ...
                    if candle_style == kestrel_loom::primitives::CandleStyle::Area {
                        // Build a semi-transparent fill from the line color
                        let fill = kestrel_loom::primitives::Color {
                            r: line_color.r,
                            g: line_color.g,
                            b: line_color.b,
                            a: 0.15,
                ...
                kestrel_loom::primitives::CandleStyle::Renko { brick_size: _ } => {
        ...
        if self.state.options.show_sessions && !self.state.options.sessions.is_empty() {
            ...
            if tf_secs <= 3600 {
                ...
                for session in &self.state.options.sessions.clone() {
                    let line_color = kestrel_loom::primitives::Color::rgba(
                        session.color.0,
                        session.color.1,
                        session.color.2,
                    ...
                    while day <= last_day {
                        ...
                        if session.show_close {
                            ...
                            if close_ts >= time_start && close_ts <= time_end {
                                ...
                                if x >= 0.0 && x <= chart_width {
                                    let dashed_color = kestrel_loom::primitives::Color::rgba(
                                        session.color.0,
                                        session.color.1,
                                        session.color.2,
        ...
        self.state.viewport.dimensions.height = full_height;
