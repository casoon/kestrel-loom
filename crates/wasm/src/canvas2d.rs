    ...
    pub fn execute_command(&mut self, cmd: &RenderCommand) {
        match cmd {
            ...
            RenderCommand::Circle {
                ...
                Canvas2DRenderer::draw_circle(self, *x, *y, *radius, *color);
            }
            RenderCommand::Ellipse {
                ...
                if let Some(fill) = style.fill_color {
                    Canvas2DRenderer::fill_ellipse(self, *cx, *cy, *rx, *ry, fill);
                }
                if let Some(stroke) = style.stroke_color {
                    Canvas2DRenderer::stroke_ellipse(
                        self,
                        *cx,
                        *cy,
                        *rx,
...
impl Renderer for Canvas2DRenderer {
    ...
    fn fill_ellipse(&mut self, cx: f64, cy: f64, rx: f64, ry: f64, color: Color) {
        Canvas2DRenderer::fill_ellipse(self, cx, cy, rx, ry, color)
    }

    fn stroke_ellipse(&mut self, cx: f64, cy: f64, rx: f64, ry: f64, color: Color, width: f32) {
        Canvas2DRenderer::stroke_ellipse(self, cx, cy, rx, ry, color, width)
    }
    ...
    fn draw_indicator_line(
        ...
        color: Color,
        width: f32,
        style: kestrel_loom::rendering::LineStyle,
    ) {
        if points.len() < 2 {
            return;
        }
        ...
        match style {
            kestrel_loom::rendering::LineStyle::Solid => {
                self.ctx.set_line_dash(&js_sys::Array::new()).ok();
            }
            kestrel_loom::rendering::LineStyle::Dashed {
                dash_length,
                gap_length,
            } => {
            ...
            kestrel_loom::rendering::LineStyle::Dotted => {
